...
public:
    ...
    std::string getStrConstValue(int tag, string value) {
        string res = value;
        if (Tags.find(tag) != Tags.end()) {
            int val = std::stoi(value);
            for (const TagValue& tagValue : Tags[tag]) {
                if (tagValue.numericValue == val) {
                    res += "(" + tagValue.constant + ")";
                    return res;
                }
            }
        }
    ...
    {
        TiffTag tempTag(tag, getTagName(tag), getTypeName(type), count, getStrConstValue(tag, res));
        return tempTag;
    }
